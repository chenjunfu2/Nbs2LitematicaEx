...
struct NoteVal
{
	std::unordered_map<MyNote2, size_t> mapNoteSubIndex;
	std::vector<MyNote2> listNoteSubMap;
	std::vector<size_t> listEncodeNoteSub;
	size_t szInstrumentCount = 0;
};

NoteVal ToNoteVal(const MyNoteList2 &listNote2)
{
	NoteVal valNote;
	std::unordered_set<NBS_File::BYTE> setInstrument;
	for (const auto &it : listNote2)
	{
		...
		//统计不同的音色种类
		if (setInstrument.emplace(it.instrument).second)
		{
			++valNote.szInstrumentCount;
		}

		//统计不同的音符盒种类并分配调色板id
		auto [itEmplace, bSuccess] = valNote.mapNoteSubIndex.try_emplace(it, valNote.listNoteSubMap.size());//如果插入成功，那么size相当于index（因为下面插入）
		if (bSuccess)
		{
		...
		valNote.listEncodeNoteSub.push_back(itEmplace->second);//插入编码数组中，这样编码数组利用size值替代了listNote2
	}
